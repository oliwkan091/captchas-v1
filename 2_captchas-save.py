...
for q in range(0, 1000):
    ...
    def checkforerror(wow):
        ...
        total = (binary[wow[0] - 2:wow[1] + 2, wow[2] - 2:wow[3] + 2]).shape[0] * \
                (binary[wow[0] - 2:wow[1] + 2, wow[2] - 2:wow[3] + 2]).shape[1]
        div = white / total
        if div > 0.63:
    ...
    while a < desired:
        try:
            ...
            continue
    plt.show()
    print('file name', file, 'q', q)
    inp = str(input('Keep it?'))
    if inp == 'y' or inp == "Y" or inp == "":
        fileskeep.append(file)